...
ctl_parse([L1,L2,L3 | T]) ->
	...
	[$ | Arg] -> ctl_parse(list_to_atom([C1,C2,C3]),Arg);
	[C4] -> ctl_parse(list_to_atom([C1,C2,C3,alpha(C4)]),[]);
	[C4,$  | Arg] -> ctl_parse(list_to_atom([C1,C2,C3,alpha(C4)]),Arg);
	_ -> error
    end;
ctl_parse(_) -> error.
...
%% Change working direcory we must keep an absoulte path (emulated
...
cdup(_Arg, S, St) ->
    ...
    rsend(S, 200, "directory changed to \"" ++ DirA ++ "\""),
    St#cstate { wd = DirR }.
...
%%
%% File Data and number formating
%%
fmt_month(1) -> "Jan";
fmt_month(2) -> "Feb";
fmt_month(3) -> "Mar";
